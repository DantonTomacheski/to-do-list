...
// Mock para o caminho da imagem
vi.mock("../assets/avatar/2.png", () => "mocked-avatar-path");

// First define the mocks - these need to be before any module imports
vi.mock("@/utils/imageCompression", () => ({
  compressImageToBase64: vi
    .fn()
    .mockResolvedValue("test-compressed-image-base64"),
}));
...
const mockUpdateUser = vi
  .mocked(await import("@/store/userStore"))
...
describe("PhotoUpload", () => {
  it("renders with default avatar when no value is provided", () => {
    ...
    expect(defaultAvatar).toHaveAttribute("src", "/src/assets/avatar/2.png");
  ...
  it("calls onChange and updates user store when file is uploaded", async () => {
    ...
    expect(mockUpdateUser).toHaveBeenCalledWith({
      photo: "test-compressed-image-base64",
    });
  });
  ...
  it("applies custom className when provided", () => {
    const handleChange = vi.fn();
    render(<PhotoUpload onChange={handleChange} className="custom-class" />);

    // Look for the main container and check its classes
    const container = screen
      .getByRole("button", { name: "uploadPhoto" })
